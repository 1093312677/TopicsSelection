		...
		<div class="row">
			...
			<div class="col-md-6">
				...
				<form action="<%=request.getContextPath()%>/topic/addTopic.do" method="post" enctype="multipart/form-data" onsubmit="return check()">
			      ...
			      <!-- 
			        <div class="input-group">
			            <span class="input-group-addon" style="border-radius: 0;">适用于学院<span style="color:red;margin-top:13px;">*</span></span>
			            <select name="" class="form-control" onchange="getGrades()" id="collegeId">
			            	<option>--选择学院--</option>
			            	<c:forEach items="${colleges }" var="items">
			            		<option value='<c:out value="${items.id }"></c:out>'><c:out value="${items.collegeName }"></c:out></option>
			            	</c:forEach>
			            </select>
			        </div>
			        <br>
			        ...
			        <div class="input-group">
			            <span class="input-group-addon" style="border-radius: 0;">适用于　系<span style="color:red;margin-top:13px;">*</span></span>
			            <div id="department-content">
			            	<select id="department" class="form-control" onchange="getDirection()">
				            	<option>--选择系--</option>
				            </select>
			            </div>
			        </div>
			       ...
			       -->  
			        <div class="input-group">
			            <span class="input-group-addon" style="border-radius: 0;">适用于年级<span style="color:red;margin-top:13px;">*</span></span>
			            <div id="grade-content">
			            	<select id="grade" name = "gradeId" class="form-control" onchange="change()" required>
				            	<option value="null">--选择年级--</option>
				            	<c:forEach items="${grades }" var="items">
			            			<option value='<c:out value="${items.id }"></c:out>'><c:out value="${items.gradeName }"></c:out></option>
				            	</c:forEach>
				            </select>
			            </div>
			        </div>
			        <br>
			         <div class="input-group">
			            ...
			            <div id="direction-content" style="z-index:1">
			            	<select id="direction" name="directionIds" class="selectpicker show-tick form-control" multiple data-live-search="true">
	                        </select>
			            </div>
			        </div>
			        <br>
