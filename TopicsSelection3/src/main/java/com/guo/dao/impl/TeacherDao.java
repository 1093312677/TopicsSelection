 ...
 */
	...
	public List<Teacher> teachers(String primaryKey, String findby) {
		...
		try{
			session=getSession();
			session.beginTransaction();
			String hql="from Teacher tea where tea.name=:primaryKey";
			if(findby.equals("no")) hql="from Teacher tea where tea.no=:primaryKey";
			Query query=session.createQuery(hql);
			query.setString("primaryKey", primaryKey);
